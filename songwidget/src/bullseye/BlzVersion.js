...
Blz.Version = Class.create();
Blz.Version.prototype = {
  initialize: function(major,minor,revision,buildNumber) {
  ...
  parse: function(versionText) {
    ...
    switch (vars.length) {
      case 4: this.buildNumber  = vars[3]; // no break
      case 3: this.revision   = vars[2]; // no break
      case 2: this.minor      = vars[1]; // no break
      case 1: this.major      = vars[0]; break;
    }
  },
  
  toString: function() {
    // "1.0.0.0";
    return this.major + "." + this.minor + "." + this.revision + "." + this.buildNumber;
  },
  ...
  compare: function(version) {
    ...
    if ( 
      ...
      (this.major == version.major && this.minor == version.minor && this.revision == version.revision && this.buildNumber < version.buildNumber)
      )
      return -1;
